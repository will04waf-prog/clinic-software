...
async function sendReminder(consultation: any, type: '24h' | '2h', supabase: any) {
  ...
  const smsBody = type === '24h'
    ? `Hi ${contact.first_name}, just a reminder that you have a consultation with ${org?.name ?? 'us'} tomorrow at ${dateStr}. Reply STOP to opt out.`
    : `Hi ${contact.first_name}, your consultation with ${org?.name ?? 'us'} is in about 2 hours (${dateStr}). See you soon!`
  ...
  try {
    ...
    if (contact.email && !contact.opted_out_email) {
      await sendEmail({
        ...
        html: wrapEmailHtml(emailBody, org?.name ?? 'Clinic'),
      })
    }
  } catch (err) {
